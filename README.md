# NYC Tech Week 2026 — Personal Agenda

Personal event tracker for [NYC Tech Week](https://www.tech-week.com/calendar/nyc) (Jun 1–4, 2026), presented by a16z.

**Live page:** https://tongliutl.github.io/nyc-tech-week-2026/
**Share page:** https://tongliutl.github.io/nyc-tech-week-2026/share.html

## What's in here

| File | Purpose |
|---|---|
| `index.html` | Full agenda — events, statuses, route maps, status board |
| `share.html` | Public-facing schedule (no status badges) |
| `.github/workflows/deploy.yml` | Auto-deploys to GitHub Pages on every push to `main` |

## Events tracked

| Day | Time | Event | Status |
|---|---|---|---|
| Mon Jun 1 | 8:30am | Maersk: The Future of Supply Chain | ✅ Done |
| Mon Jun 1 | 12:30pm | IBM Generative Computing Masterclass | ✅ Done |
| Mon Jun 1 | 2:00pm | IBM Beyond the Spec: Engineering in the Age of Agents | ✅ Done |
| Tue Jun 2 | 9:00am | Agentic Commerce: Build Day | ✅ Done |
| Tue Jun 2 | 4:00pm | Operating MCP | ✅ Done |
| Tue Jun 2 | 6:00pm | Cloudflare + Shopify: Build for the Agent Era | ✅ Done |
| Wed Jun 3 | 10:45am | Architecture of Failure: 401-Level AI Agents Masterclass | ✅ Done |
| Wed Jun 3 | 1:00pm | Google Ads Keynote: Future of AI Marketing | ✅ Done |
| Wed Jun 3 | 2:30pm | Google AI Learning Lab: Build Your Own AI Agent | ✅ Done |
| Thu Jun 4 | 9:00am | Snowflake Cafe at Verci | ✅ Approved |
| Thu Jun 4 | 1:00pm | Autonomously Improving Agent Swarms with Weights & Biases | ✅ Approved |

## Leave-home times

Route maps in the page show exact timing sourced from MTA GTFS + OpenStreetMap routing.

| Day | Leave home | Train | GCT arrive | Subway to venue |
|---|---|---|---|---|
| Mon Jun 1 | 6:54am | dep 7:09am | 7:46am | 4/5/6 → 14th St (~15 min) |
| Tue Jun 2 | 7:20am | dep 7:35am | 8:10am | 6 → Spring St (~18 min) |
| Wed Jun 3 | 8:59am | dep 9:14am | 9:48am | S → Times Sq → 1 (~15 min) |
| Thu Jun 4 | 7:50am | dep 8:05am | 8:38am | 4/5/6 → 23rd St (~10 min) → Verci; then 4/5/6 → Brooklyn Bridge (~25 min) → W&B 1pm |
